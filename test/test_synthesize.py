...
def chiptunes_synthesize(midi, fs=44100):
    ...
    for inst in midi.instruments:
        ...
        if inst.is_drum:
            ...
            print(pretty_midi.program_to_instrument_name(inst.program))
            is_bass = (
    ...
    m = np.max(np.abs(synthesized))
    synthesized = (synthesized/m).astype(np.int16)
    return synthesized


if __name__ == '__main__':
    print( "Synthesizing {} ...".format('midi'))
    # Load in MIDI data and synthesize using chiptunes_synthesize
    midi_object = pretty_midi.PrettyMIDI('SynthBass1.mid')
    synthesized = chiptunes_synthesize(midi_object, 44100)
    print( "Writing {} ...".format('wav'))
    # Write out
    scipy.io.wavfile.write(
        'chiptunes_0922.wav', 44100, synthesized)
    synthesized_midi = midi_object.synthesize()
    scipy.io.wavfile.write(
        'chiptunes_0922_1.wav', 44100, synthesized_midi)
